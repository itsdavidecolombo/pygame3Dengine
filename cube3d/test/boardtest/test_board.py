...
class TestGameBoard(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.board = board_mock.make_board_mock(player = object(), logger = object())
    ...
    def test_should_raise_ValueError_if_none_objects_in_constructor(self):
        try:
            board_mock.make_board_mock(player = None, logger = object())
            self.fail()
        except ValueError as ex:
            print(ex.__str__())

        try:
            board_mock.make_board_mock(player = object(), logger = None)
            self.fail()
        except ValueError as ex:
            print(ex.__str__())
